...
int main(int argc, char *argv[]) {
	...
	for(i = 4; i < argc; i++) {	// add songs to the array
		insertArray(&stations, base_multicast_addr + htonl((i-4)));				// insert the multicast addresses acording to the input
		lastStation = i-4;
	...
	sender_socket_fd = socket(AF_INET, SOCK_DGRAM, 0);	// setup the socket
				...
				if(permitSong == 0) {
					...
					if(access(songName, F_OK) != -1) {				// check if the song already exists
						...
						if(!(recSong = fopen(songName, "w"))) {			// open and create the new song with writing permissions
							...
							permitUpload(who, 1);		// permit song upload
