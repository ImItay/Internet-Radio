...
int main(int argc, char *argv[]) {	
	...
	// clear our the sender_addr tcp_buffer
	...
	tcp_buffer[0] = hello.commandType;						// send the first HELLO message
	*(uint16_t *) (&(tcp_buffer)[1]) = hello.reserved;
	
	send(tcp_socket_fd, &tcp_buffer, 3, 0);
...
void Main_listener() {
	...
	while(tcpFlag) {
		...
		if(selectValue < 0 && (errno != EINTR)) {
			...
			error("Timeout occured!, disconneting");
		}
		else {
			wait = 0;		// disable setting timeout
		}
		
		if(FD_ISSET(stdin_fd, &fds)) {		// USER INPUT
			...
			switch(userAction) {
				case 1: /* Ask_Song -- change station */
						...
						if(wantedStation >= numStations) {
							printf("Please retry by entring a valid station number, or 'q' to quit.\n");
						}
						else {
		...
		else if(FD_ISSET(tcp_socket_fd, &fds)) {		// TCP socket
			...
			switch(tcp_buffer[0]) {
				case 1: 			// Announce - reply for askSong
					if(!waitAnnounce) {
						...
						change_station = 1;				// change the flag to change station
				...
				case 3:			// Inavlid_command
					invalidStr = (char *) malloc(tcp_buffer[1]);
					strncpy(invalidStr, (tcp_buffer+2), tcp_buffer[1]);
					error(invalidStr);					
				break;
				
				case 4:			// New Stations detected
					//numStations = (*(uint16_t *) tcp_buffer+1);
					numStations ++ ;
					if(bytes_rec != 3) {
...
void UDP_listener() {
	...
	fp = popen("play -t mp3 -> /dev/null 2>&1", "w");	// open pipe and redirect "stderr" output to "stdout"
